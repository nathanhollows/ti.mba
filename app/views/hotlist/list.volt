...
<div class="row">
    <div class="visible-sm visible-xs">
        <div class="alert alert-warning">
            ...
            <strong>You're using mobile!</strong> Would you like to switch to the list view instead?
            <br>
            <a href="/hotlist/list" class="btn btn-info">List View</a>
        </div>
    </div>
</div>
</div>
	...
	<div class="category {% if category.jobs|length < 1 %}hidden{% endif %}">
		<ul class="list-group searchable" id="list{{ category.id }}">
			...
			{% for job in category.jobs %}
                ...
                <strong>{{ job.title|escape }} <span class="pull-right">${{ job.value|number }}</span></strong>
                <br>
