				...
				<div class="btn-group" role="group" aria-label="Button group with nested dropdown">
					...
					<div class="btn-group" role="group">
						...
						<div class="dropdown-menu dropdown-menu-right" aria-labelledby="btnGroupDrop1">
							{{ linkTo(['quotes/new/?company=' ~ customer.customerCode, 'Create New Quote', 'class':
							'dropdown-item']) }}
							{{ linkTo(['customers/details/' ~ customer.customerCode, 'Print Contacts List', 'class':
							'dropdown-item']) }}
							{{ linkTo(['customers/history/' ~ customer.customerCode, 'Print Contact Records', 'class':
							'dropdown-item']) }}
						</div>
					</div>
				</div>
	...
	<div class="row">
			...
			<div class="card shadow-sm mb-3">
				<div class="card-body">
					<h5 class="card-title">Details
						<a class="float-right open-modal text-sm text-muted" data-target="#modal-ajax" href='{{ url('
							customers/edit/' ~ customer.customerCode) }}' data-target="#modal-ajax">
							{{ emicon("pencil") }}
						</a>
					</h5>
				</div>
				<ul class="list-group list-group-flush mt-n4">
					<li class="list-group-item">
						...
						<span class="badge badge-{{ customer.state.style }} float-right">{{ customer.state.name
							}}</span>
						{% else %}
					...
					<li class="list-group-item">
						...
						<a href="tel:{{ customer.phone|stripspace }}" class="tel-link float-right">{{ customer.phone
							}}</a>
					</li>
					<li class="list-group-item">
					...
					<li class="list-group-item">
						<span class="title">Email</span> <a href="mailto:{{ customer.email }}"
							class="float-right email">{{ customer.email }}</a>
					</li>
					...
					<li class="list-group-item"><span class="title">Sales Area</span>
						...
						<span class="float-right">{{ linkTo("customers/region/" ~ customer.salesarea.nicename,
							customer.salesarea.name ) }}</span>
						{% else %}
						<span class="badge badge-secondary float-right">Not set</span>
						{% endif %}
					</li>
		...
		<div class="col-sm-12 col-lg-9">
			<ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
				<li class="nav-item">
					<a class="nav-link active" id="pills-timeline-tab" data-toggle="pill" href="#pills-timeline"
				...
				<li class="nav-item">
					<a class="nav-link" id="pills-contacts-tab" data-toggle="pill" href="#pills-contacts" role="tab"
						aria-controls="pills-contacts" aria-selected="false">Contacts</a>
				</li>
				<li class="nav-item">
					<a class="nav-link" id="pills-quotes-tab" data-toggle="pill" href="#pills-quotes" role="tab"
						aria-controls="pills-quotes" aria-selected="false">Quotes {% if customer.activeQuotes|length > 0
						%}<span class="badge badge-primary text-light">{{ customer.activeQuotes|length}}</span>{% endif
						%}</a>
				</li>
				<li class="nav-item">
					<a class="nav-link" id="pills-orders-tab" data-toggle="pill" href="#pills-orders" role="tab"
						aria-controls="pills-orders" aria-selected="false">Orders <span
							class="badge badge-primary text-light">{{ customer.orders|length }}</span></a>
				</li>
			</ul>
			<div class="tab-content" id="pills-tabContent">
				...
				<div class="tab-pane fade" id="pills-contacts" role="tabpanel" aria-labelledby="pills-contacts-tab">
					<div class="card shadow-sm">
						<div class="card-body">
							...
							<div class="btn-group float-right ml-3">
								...
								<a class="btn btn-sm btn-primary open-modal"
									href="/contacts/new/{{ customer.customerCode }}" role="button">
									{{ emicon("plus") }}
									Add
								</a>
							</div>
							<input id="contact-search" name="search" placeholder="Search..." type="text"
								data-list="#contacts-list" class="form-control w-25 float-right shadow-sm"
								autocomplete="off">
						</div>
						{% set role = "h" %}
						<ul class="list-group list-group-flush" id="contacts-list">
							...
							<li
								class="list-group-item bg-light font-weight-bold border-left-0 border-right-0 border-top-0 header">
								{% if contact.job %}{{ contact.job.name }}{% else %}Misc{% endif %}</li>
							{% set role = contact.role %}
							{% endif %}
							<li class="list-group-item">
								<div class="row">
									<div class="col-xs-12 col-md-4">
										<span class="xedit-toggle" data-name="name" data-type="text"
											data-pk="{{ contact.id }}" data-url="/contacts/ajaxupdate"
											data-placement="auto" data-title="Name">
											{{ contact.name }}
										</span>
										<span class="xedit-toggle xedit-hide float-right" data-name="role"
											data-type="select" data-pk="{{ contact.id }}"
											data-url="/contacts/ajaxupdate" data-placement="auto" data-title="Role"
											data-value="{{ contact.role }}" data-source='{{ roles }}'></span>
									</div>
									<div class="col-xs-12 col-md-3">
										<a href="tel:{{ contact.directDial|stripspace }}" class="xedit-toggle tel-link"
											data-name="directDial" data-type="tel" data-pk="{{ contact.id }}"
											data-url="/contacts/ajaxupdate" data-placement="auto"
											data-title="Direct Dial">{{ contact.directDial }}</a>
									</div>
									<div class="col-xs-12 col-md-5">
										<a href="mailto:{{ contact.email }}" class="xedit-toggle" data-name="email"
											data-type="text" data-pk="{{ contact.id }}" data-url="/contacts/ajaxupdate"
											data-placement="auto" data-title="Email">{{ contact.email }}</a>
										<a href="/contacts/delete/{{ contact.id }}" tabindex="-1"
											class="text-danger confirm-delete float-right">
											{{ emicon("trash-2") }}
										</a>
									</div>
								</div>
					...
					<table class="table table-hover rounded bg-white shadow-sm">
						...
						<tbody>
							...
							<tr>
								...
								<td> <span class="badge badge-{{ quote.state.style }}">{{ quote.state.name }}</span>
								</td>
							</tr>
							{% endfor %}
						</tbody>
				...
				<div class="tab-pane fade" id="pills-orders" role="tabpanel" aria-labelledby="pills-orders-tab">
					...
					<div class="card shadow-sm mb-3">
						<div class="card-body">
							<h5 class="card-title">
								Order {{ linkTo("orders?customer="~order.customerCode~"&order="~order.orderNumber,
								order.orderNumber ) }}
							</h5>
					...
					<div class="text-center">
						<strong>{{ linkTo(["orders?customer=" ~ customer.customerCode, "See historic orders", "class":
							"mt-3"]) }}</strong>
					</div>
				</div>
			</div>
...
<style>
	.email {
		white-space: nowrap;
		/* Keeps the email on one line */
		overflow: hidden;
		/* Ensures the overflow is not visible */
		text-overflow: ellipsis;
		/* Adds the ellipsis for overflowed text */
		max-width: 200px;
		/* Or whatever maximum width you prefer. Adjust based on your layout needs. */
