	...
	<div class="header py-3">
		<div class="row header-body">
			<div class="col">
				...
				<h6 class="header-pretitle">Apr {{ date("Y", strtotime(date)) }} - Mar {{ date("Y", strtotime(date ~ " +1 year")) }}</h6>
			</div>
			<div class="col text-right">
				<div class="btn-group" role="group" aria-label="Change report year">
					{{ linkTo(["customers/regions/" ~ (year - 1), year - 1, "class": "btn btn-info"]) }}
					{{ linkTo(["customers/regions/" ~ (year), year, "class": "btn btn-info active"]) }}
					{% if date("Y") >= (year + 1) %}
					{{ linkTo(["customers/regions/" ~ (year + 1), year + 1, "class": "btn btn-info"]) }}
					{% endif %}
	...
	<div class="col">
		<table class="table table-hover bg-white shadow">
			...
			<tbody>
				...
				<tr>
					<td class="pl-3">
						{{ linkTo("customers/region/" ~ region.nicename, region.name) }}
					</td>
					...
					<td class="text-right">
						{% if index < figures|length %}
							{% if date("Y-m", strtotime(date)) ==
								figures[index].period %}
