...
class FreightController extends ControllerBase
{
	...
	public function importAction()
	{
        ...
        // Runs python script to import docket data from access database
        exec('C:\xampp\htdocs\app\script\import_freight.py', $output, $return);

        if (!$return) {
            $this->flashSession->success("Dockets imported successfully from TS");
        } else {
            $this->flashSession->error("The dockets could not be imported from TS");
        }
        $this->_redirectBack();
